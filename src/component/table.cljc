(ns component.table
  (:require 
   ...
   [ui.table :refer [table-ui]]
   [replicant.query :as query]))

(defn select-row [{:keys [row event selected? table-id row->id]}]
  [[:store/assoc-in [:tables table-id :selected (row->id row)] selected?]])

(defn select-all [{:keys [table-id event selected? rows row->id]}]
  [[:store/assoc-in [:tables table-id :selected] (into {} (map (fn [row]
                                                                 [(row->id row) selected?])
                                                               rows))]])
...
(defn execute-action [store event action args]
  (case action 
    ...
    :table/select-all (select-all args)
    nil))
...
(defn get-selected-ids [state table-id] 
  (into #{} (keys
             (filter (fn [[_ v]] v)
                     (get-in state [:tables table-id :selected] {})))))

(defn get-selected-rows [state table-id query-kind row->id]
  (let [rows (get-rows state table-id query-kind)
        selected-ids (get-selected-ids state table-id)]
    (filter (fn [row] (contains? selected-ids (row->id row))) rows)))

;; TODO: table 的所有 config 应该都写到 store 里面?
...
(defn long-filter [state filter-key label placeholder table-id]
  (let [value (get-in state [:tables table-id :filters filter-key])]
    [:div {:class ["form-control"]}
     [:label {:class ["label"]}
; [:app] Build completed. (233 files, 7 compiled, 0 warnings, 0.11s)
; [:app] Compiling ...
      [:span {:class ["label-text"]} label]]
     [:label {:class ["input"]}
      [:input {:type "number"
